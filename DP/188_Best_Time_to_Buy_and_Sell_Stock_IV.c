...
int maxProfit(int k, int* prices, int pricesSize) {
        ...
        int days = pricesSize;
        if (k >= days) return maxProfit2(prices, pricesSize);
        
        int**local = (int**)malloc(days*sizeof(int*));
        int** global = (int**)malloc(days*sizeof(int*));
        for(int i =0; i<days; i++){
            local[i] = (int*) malloc((k+1)*sizeof(int));
            global[i] = (int*) malloc((k+1)*sizeof(int));
        }
        for(int i =0; i<days; i++){
            for(int j =0; j<=k;j++){
                local[i][j] =0;
        ...
        for (int i = 1; i < days ; i++) {
            ...
            for (int j = 1; j <= k; j++) {
                local[i][j] = MAX(global[i - 1][j - 1], local[i - 1][j] + diff);
                global[i][j] = MAX(global[i - 1][j], local[i][j]);
               // printf("i=%d j=%d local=%d, global=%d\n", i, j, local[i][j], global[i][j]);
             }
        }
